...
public class RouteFinderImpl implements RouteFinder {
	...
	private RouteFinderDistanceHelper finderDistanceHelper = new RouteFinderDistanceHelper();
	...
	public void computePaths(final Vertex source) throws RouteFinderException {
		...
		finderDistanceHelper.setDistance(new HashMap<Vertex, Integer>());
		predecessors = new HashMap<Vertex, Vertex>();
		finderDistanceHelper.getDistance().put(source, 0);
		unSettledNodes.add(source);
		while (unSettledNodes.size() > 0) {
			final Vertex node = finderDistanceHelper.getMinimum(unSettledNodes);
	...
	private void findMinimalDistances(final Vertex node) {
		...
		try {
			...
			for (Vertex target : adjacentNodes) {
				if (finderDistanceHelper.getShortestDistance(target) > finderDistanceHelper.getShortestDistance(node) + getDistanceBetweenNodes(node, target)) {
					finderDistanceHelper.getDistance().put(target, finderDistanceHelper.getShortestDistance(node) + getDistanceBetweenNodes(node, target));
